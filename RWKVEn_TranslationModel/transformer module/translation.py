...
def readLangs(lang1, lang2, reverse=False):
    ...
    #/home/thulasi/UPEKSHA/RWKV/data/eng-fra.txt
...
def prepareData(lang1, lang2, reverse=False):
  # /content/data/data/eng-fra.txt
    input_lang, output_lang, pairs = readLangs(lang1, lang2, reverse)
    print("Read %s sentence pairs" % len(pairs))
    pairs = filterPairs(pairs)    #--do not filter anything
...
class EncoderRNN(nn.Module):
    def __init__(self, rwkv_model, input_size, hidden_size, dropout_p=0.1):
        super(EncoderRNN, self).__init__()
        self.rwkv_model = rwkv_model  # Pass your pre-initialized RWKV model here
    ...
    def forward(self, input):
        ...
        rwkv_output = self.rwkv_model(embedded)
        # Return RWKV outputs (modify based on RWKV's behavior)
        return rwkv_output, None  # Replace None with RWKV hidden state if it exists
...
def train_epoch(dataloader, encoder, decoder, encoder_optimizer,
    ...
    for data in dataloader:
        ...
        encoder_outputs, e1,e2 = encoder(input_tensor)
        encoder_hidden = (e1+e2)/2
...
def evaluateRandomly(encoder, decoder, n=2000):
...
seq_len = 256  # Match the `ctx_len` of the encoder
hidden_size = 128
batch_size = 32
# Then when you run forward:
input_tokens = torch.randint(0, input_lang.n_words, (batch_size, seq_len)).to(device)
rwkv_out, e1, e2 = encoder(input_tokens)
encoder_hidden =(e1+e2)/2

print("Encoder output:", rwkv_out)

print("Encoder output shape:", rwkv_out.shape)
print("Returned e1, e2 shape:", e1.shape, e2.shape)
...
# hidden_size = 128
# batch_size = 32
rwkv_model = L2Wrap.apply  #  L2Wrap() does not work

input_lang, output_lang, train_dataloader = get_dataloader(batch_size)

# encoder = EncoderRNN(rwkv_model,input_lang.n_words, hidden_size).to(device)
decoder = AttnDecoderRNN(hidden_size, output_lang.n_words).to(device)



# rwkv_model = L2Wrap()  # Initialize RWKV
# encoder = EncoderRNN(rwkv_model=rwkv_model, input_size=input_size, hidden_size=hidden_size)

train(train_dataloader, encoder, decoder, 80, print_every=5, plot_every=5)
torch.save(encoder.state_dict(), 'encoder3.pth')
torch.save(decoder.state_dict(), 'decoder3.pth')
print("Models saved.") #will be saved to the current directory #should run on the same directory as cuda is installed